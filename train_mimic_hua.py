...
def readfile(filename,max_seq_len):
    ...
    print('Processing %s, lines: %d' % (filename,num_lines))
    with open(filename,'r') as f:
        reader=csv.reader(f,delimiter=',')
        next(reader)
        #for line in tqdm(reader,total=num_lines,file=sys.stdout):
        for i,line in enumerate(reader):
            prog=int(num_lines/10)
            if i and i % int(num_lines/100)==0:
                print('.',end='') 
            if i and i % int(num_lines/10)==0:
                print('%d%%' % (i // int(num_lines/10) * 10))
    ...
    print('\nProcess finished for %s' % filename)
    return examples



max_seq_len=128
train_features=readfile(filename,max_seq_len)
###
BATCH_SIZE=32
EPOCHS=20
LR=2e-5
...
##get eval data
eval_features=readfile(filename2,max_seq_len)
...
def get_roc(res,filename):
    ...
    prec_rec_df=prec_rec_df[prec_rec_df.precision>0.799999].reset_index()
    rp80=np.NaN
...
for epoch in range(EPOCHS):
    ...
    for step,batch in enumerate(train_loader):
        ...
        if step and step % 2000 == 0:
            ...
            ###plot train loss history up till current time
            ...
            pred_probs=[]
            admids=[]
            #for adm_ids,input_ids,input_mask,segment_ids,labels in tqdm(eval_loader,file=sys.stdout):
            for adm_ids,input_ids,input_mask,segment_ids,labels in eval_loader:
                if hasgpu:
            ...
            res.to_csv('pred_res_epoch_{}_step{}.csv'.format(epoch,step),index=False)
...
def timetostr(num):
    ...
    mins=(num % 3600)//60
    secs=(num % 3600) % 60
    return '{} hours {} mins {} secs'.format(hrs,mins,secs)
print('\n\nJob started at %s\nJob ends at %s\nJob takes total of %s\n' % (startdate,enddate,timetostr(endtime-starttime)))
